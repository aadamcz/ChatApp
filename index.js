...
io.on("connection", function(socket) {
	//nasłuchiwanie na połączenie klienta z aplikacją. arg socket reprezentuje osobę, która właśnie weszła do czatu
	//miejsce dla funkcji, które zostaną wykonane po podłączeniu klienta
	//klient nasłuchuje na wiadomość wejścia do czatu
	socket.on("join", function(name) {
		//użytkownika, który pojawił się w aplikacji zapisujemy do serwisu trzymającego listę osób na czacie
		userService.addUser({
			id: socket.id,
			name
		});
